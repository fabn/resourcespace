...
function InfoBoxMM(event)
    {
   	var i=$('InfoBoxCollection');
   	if (!i) {return false;} // no object? ignore for now
   	
   	var ii=$('InfoBoxCollectionInner');
    var x=event.clientX;
    var y=event.clientY;
...
function InfoBoxAppear()
	...
	if ((InfoBoxWaiting) && (InfoBoxRef!=0))
    	...
    	//Ajax loader here
    	$('InfoBoxCollectionInner').innerHTML='';
    	new Ajax.Updater('InfoBoxCollectionInner','ajax/infobox_loader.php?ref=' + InfoBoxRef,{ method: 'get' });
