var queued_too_many_files = "";
var creatingthumbnail = "";
var uploading = "";
var thumbnailcreated = "";
var done = "";
var stopped = "";
	...
	try {
		...
		switch(error_code) {
			case SWFUpload.ERROR_CODE_QUEUE_LIMIT_EXCEEDED:
				error_name = queued_too_many_files;
			break;
		}
...
function uploadProgress(fileObj, bytesLoaded) {
	...
	try {
		...
		if (percent === 100) {
			progress.SetStatus(creatingthumbnail);
...
function uploadSuccess(fileObj, server_data) {
	try {
		...
		progress.SetStatus(thumbnailcreated);
		progress.ToggleCancel(false);
