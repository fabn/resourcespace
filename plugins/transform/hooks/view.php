...
function HookTransformViewAfterresourceactions (){
	...
	if (!isset($cropper_formatarray)){
		$cropper_formatarray = array('TIF','TIFF','JPG','JPEG','PNG','GIF','BMP','PSD'); // file formats that can be transformed
	} else {
		// in case these have been overriden, make sure these are all in uppercase.
		for($i=0;$i<count($cropper_formatarray);$i++){
			$cropper_formatarray[$i] = strtoupper($cropper_formatarray[$i]);
		}	
	}

	if ($access==0 && $resource['has_image']==1 && in_array(strtoupper($resource['file_extension']),$cropper_formatarray)){
