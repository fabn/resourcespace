	...
	#$headers .= "X-Sender:  x-sender" . $eol;
   	...
   	for ($n=0;$n<count($reply_tos);$n++){
		...
		if (strstr($reply_tos[$n],"<")){ 
			...
			$headers.=mb_encode_mimeheader($from_name, "UTF-8", "Q") . " <".$reply_tos[$n].">";
		}
 	}
 	$headers.=$eol;
	...
	if ($cc!=""){
		...
		for ($n=0;$n<count($ccs);$n++){
			...
			if (strstr($ccs[$n],"<")){ 
				...
				$headers.=mb_encode_mimeheader($userfullname, "UTF-8", "Q"). " <".$ccs[$n].">";
			}
		}
		$headers.=$eol;
	}
...
function send_mail_phpmailer($email,$subject,$message="",$from="",$reply_to="",$html_template="",$templatevars=null,$from_name="",$cc="",$html=false)
	...
	global $use_smtp,$smtp_secure,$smtp_host,$smtp_port,$smtp_username,$smtp_password;
	$mail = new PHPMailer();
